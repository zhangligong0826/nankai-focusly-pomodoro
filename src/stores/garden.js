...
export const useGardenStore = defineStore('garden', () => {
  ...
  /** 已解锁品种 id 列表 */
  const unlocked = ref(['sprout'])
  /** 当前植株 */
  const plant = ref({ speciesId: 'sprout', wilted: false, wiltedAt: null })
  /** 累计收获次数（开花后结算） */
   ...
   */
  function init() {
    const saved = storage.getItem(LS_KEY.GARDEN, null)
    if (saved && typeof saved === 'object') {
      if (Array.isArray(saved.unlocked) && saved.unlocked.length) {
        unlocked.value = saved.unlocked
      }
      if (saved.plant && typeof saved.plant === 'object') {
        plant.value = { ...plant.value, ...saved.plant }
      }
      harvestedCount.value = saved.harvestedCount || 0
      lastStreak.value = saved.lastStreak || 0
    }
    inited.value = true
  }
