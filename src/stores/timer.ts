...
import type { TimerMode, TimerStatus, SessionType } from '@/types'

/** 模块级（非响应式）：定时器 id 与本次会话的截止时间戳 */
let intervalId: ReturnType<typeof setInterval> | null = null
let deadlineAt: number | null = null

/** 专注会话记录（含 P1-3 意图/反思与 P1-6 中止标记） */
...
export const useTimerStore = defineStore('timer', () => {
  ...
  /** 当前计时模式 */
  const mode = ref<TimerMode>(TIMER_MODE.FOCUS)
  /** 运行状态 */
  ...
  /** 阶段切换提醒（BreakReminder 订阅） */
  const phaseReminder = ref<PhaseReminder | null>(null)
  /** 每日目标达成触发计数（DailyGoalCelebration 订阅） */
  const celebrationTrigger = ref(0)
  /** 最近一次完成信息 */
  const lastComplete = ref<{ type: SessionType; skipped: boolean; at: number } | null>(null)
  /** 本次专注的意图（P1-3，开始前填写，空串=未填） */
  const intent = ref('')
  /** 待反思的会话 id（P1-3，专注正常完成后触发反思弹窗） */
  const pendingReflectionId = ref<string | null>(null)
  ...
  async function complete(skipped = false) {
    ...
    const sessions = await idbGet<SessionRecord[]>(LS_KEY.SESSIONS, [])
  ...
  async function setConfig(payload: Record<string, unknown>) {
  ...
  /** 绑定当前任务 */
  function bindTask(taskId: string) {
  ...
  async function saveReflection(rating: number | null, reflection = '') {
  ...
  function init() {
    ...
    const saved = storage.getItem<Record<string, unknown> | null>(LS_KEY.TIMER_STATE, null)
    if (saved && typeof saved === 'object') {
      mode.value = (saved.mode as TimerMode) || TIMER_MODE.FOCUS
      currentRound.value = Number(saved.currentRound) || 1
      totalRounds.value = Number(saved.totalRounds) || totalRounds.value
      boundTaskId.value = (saved.boundTaskId as string | null) ?? boundTaskId.value

      if (saved.status === TIMER_STATUS.RUNNING && saved.deadlineAt) {
        // 运行中恢复：按 deadline 重算，已过期则补结算
        const remaining = computeRemaining(Number(saved.deadlineAt))
        deadlineAt = Number(saved.deadlineAt)
        if (remaining <= 0) {
      ...
      if (saved.status === TIMER_STATUS.PAUSED) {
        ...
        remainingSeconds.value =
          Number(saved.remainingSeconds) || totalSeconds.value
        status.value = TIMER_STATUS.PAUSED
        return
      }
    }
